...
class SettingsStorage {
    ...
    func save<T>(_ value: T, forKey key: String) {
        ...
        logDebug("Saved key: \(key)", log: .settings)
    }
    ...
    func remove(forKey key: String) {
        ...
        logDebug("Removed key: \(key)", log: .settings)
    }
    ...
    func loadWindowPosition() -> CGPoint? {
        guard defaults.object(forKey: SettingsKeys.windowPositionX) != nil,
              defaults.object(forKey: SettingsKeys.windowPositionY) != nil else {
            return nil
        }
    ...
    func saveAll() {
        ...
        logDebug("All settings saved", log: .settings)
    }
    ...
    func reset() {
        ...
        logInfo("All settings reset to defaults", log: .settings)
    }
    ...
    func migrateIfNeeded() {
        ...
        if version < currentVersion {
            logInfo("Migrating from version \(version) to \(currentVersion)", log: .settings)
    ...
    private func migrateToV1() {
        logDebug("Running migration to V1", log: .settings)
    }
