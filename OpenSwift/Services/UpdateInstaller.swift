...
final class UpdateInstaller {
    ...
    private func writeHelperScript(stagedApp: URL, stagingDir: URL) throws -> URL {
        ...
        let lines: [String] = [
            ...
            "# 1. wait for old process to fully exit (graceful, bounded)",
            "for i in $(seq 40); do kill -0 \"$OLD_PID\" 2>/dev/null || break; sleep 0.3; done",
            "# force kill if still alive (modal sheet blocks terminate)",
